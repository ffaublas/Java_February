package com.faublas.fullstackrest.controllers;
...
import com.faublas.fullstackrest.models.MenuItem;
import com.faublas.fullstackrest.services.MenuItemService;
...
//		//send an empty MenuItem object to the form for the form to bind to
	...
	public String update(@PathVariable("id") Long id, @Valid @ModelAttribute("itemToEdit") MenuItem menuItemToEdit, BindingResult result) {
		...
		//the @ModelAttributre("itemToEdit") needs to match the modelAttribute in the edit form so we can get the menu item object from the form and store it in a variable. If the result has errors (form was not filled out properly), then we re -render the edit form
		if(result.hasErrors()) {
			
			return "edit.jsp";
		}else { //otherwise if there's no errors, we send the object to update to the service and the service will talk to the repository to update that item
